...
void InicializarKit (void)
{
	...
	//Inicio LCD 4 bits
	LCDInit();
	//Inicio UART1 para rs232
	InitUART1 ();
	//Inicio DAC
	InitDAC();
	//Inicio Timer 1 con match 0 de 1 seg
	InitTimer1();
	//Inicializo el DMA:
	InitDMA();
...
{	//1°)Selecciono FUNCION del pin en PINSELx.
	...
	//LCD
	SetPINSEL(P0,4,PINSEL_GPIO);	//LCD E
	SetPINSEL(P2,6,PINSEL_GPIO);	//LCD RST
	SetPINSEL(P0,5,PINSEL_GPIO);	//LCD D4
	SetPINSEL(P0,10,PINSEL_GPIO);	//LCD D5
	SetPINSEL(P2,4,PINSEL_GPIO);	//LCD D6
	SetPINSEL(P2,5,PINSEL_GPIO);	//LCD D7
