...
static
int select (void)	/* TRUE:Successful, FALSE:Timeout */
{
	CS_LOW();
	if (wait_ready() != 0xFF) {
		deselect();
...
void power_on (void)
{
	...
	PINSEL_ConfigPin(&PinCfg);
	PinCfg.Funcnum = 0;
	PinCfg.Portnum = SSELPORT;
	PinCfg.Pinnum = SSELPIN;
	PINSEL_ConfigPin(&PinCfg);
...
//	SSP_ConfigStruct.ClockRate = 400000;
	...
	//CLK del periférico SSP en 100MHz, SCR = 0, CPSR=4 para hi speed CPSR=250 para low speed

	LPC_SC->PCONP |= 1<<10;
	LPC_SC->PCLKSEL0 |= 1<<20;   // PCLK_SSP1=CCLK/1
	LPC_SSP1->CR0 = 7;		// 8bit, SPI, CPOL=0, CPHA=0
	LPC_SSP1->CPSR = 250;
	/*LPC_SSP1->CR0 = 7;
	LPC_SSP1->CPSR = 62  //250*/
...
DSTATUS SDMMC_disk_initialize (
	BYTE drv		/* Physical drive nmuber (0) */
)
{
	BYTE n, cmd, ty, ocr[4];

	GPIO_SetDir(SSELPORT, 1<<SSELPIN, 1);  /* CS */
//	GPIO_SetDir(CARDDETECTPORT, 1<<CARDDETECTPIN, 0); /* Card Detect */
...
//	pv = ((GPIO_ReadValue(CARDDETECTPORT) & (1<<CARDDETECTPIN)) != 0);
